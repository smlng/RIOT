...
static gnrc_sixlowpan_nd_router_abr_t *_get_abr(const ipv6_addr_t *addr)
{
    ...
    for (int i = 0; i < GNRC_SIXLOWPAN_ND_ROUTER_ABR_NUMOF; i++) {
        if (ipv6_addr_equal(&_abrs[i].addr, addr)) {
            ...
            _abrs[i].addr.u64[0] = addr->u64[0];
            _abrs[i].addr.u64[1] = addr->u64[1];
            _abrs[i].ltime = 0;
            _abrs[i].version = 0;
            _abrs[i].prfs = NULL;
            memset(&_abrs[i].ctxs, 0, sizeof(&_abrs[i].ctxs));
            return &_abrs[i];
        }
    }
    return NULL;
                            ...
                            gnrc_sixlowpan_nd_router_abr_t *abr)
{
    ...
    DEBUG(" -- prefix %s\n", ipv6_addr_to_str(addr_str, &(pi_opt->prefix), sizeof(addr_str)));
    /* check PIO */
